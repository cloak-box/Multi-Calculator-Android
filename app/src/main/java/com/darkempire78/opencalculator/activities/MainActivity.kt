...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.pointButton.setImageResource(
      if (decimalSeparatorSymbol == ",") R.drawable.comma else R.drawable.dot
    )

    // Set history
    historyLayoutMgr = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
    binding.historyRecylcleView.layoutManager = historyLayoutMgr
    historyAdapter =
      HistoryAdapter(
        mutableListOf(),
        { value -> updateDisplay(window.decorView, value) },
    ...
    binding.slidingLayout.addPanelSlideListener(
      object : PanelSlideListener {
        ...
        override fun onPanelStateChanged(
          ...
          if (
            newState == PanelState.ANCHORED
          ) { // To prevent the panel from getting stuck in the middle
    ...
    binding.input.minWidth =
      screenWidthPX -
        (binding.input.paddingRight + binding.input.paddingLeft) // remove the paddingHorizontal

    // Do not clear after equal button if you move the cursor
    binding.input.accessibilityDelegate =
        ...
        binding.resultDisplay.text.toString() != "" -> {
          if (MyPreferences(this).longClickToCopyValue) {
            ...
            clipboardManager.setPrimaryClip(
              ClipData.newPlainText(R.string.copied_result.toString(), binding.resultDisplay.text)
            )
    ...
    binding.input.addTextChangedListener(
      object : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable?) {
          // Do nothing
          s?.let { afterInputChanged(s.toString()) }
        }
      }
    )

    binding.resultDisplay.addTextChangedListener(
      object : TextWatcher {
        private var beforeTextLength = 0
  ...
  // Displays a popup menu with options to insert double zeros ("00") or triple zeros ("000") into
  // the specified EditText when the zero button is long-pressed.
      ...
      if (errorStatus != errorStatusOld) {
        ...
        if (errorStatus) {
          ...
          binding.resultDisplay.setTextColor(
            ContextCompat.getColor(this, R.color.calculation_error_color)
          )
        }
        // Clear error color
        else {
          ...
          binding.resultDisplay.setTextColor(
            ContextCompat.getColor(this, R.color.text_second_color)
          )
        }
  ...
  private fun updateDisplay(view: View, value: String) {
    ...
    lifecycleScope.launch(Dispatchers.Default) {
      ...
      withContext(Dispatchers.Main) {
        ...
        if (
          value == decimalSeparatorSymbol && decimalSeparatorSymbol in binding.input.text.toString()
        ) {
          if (binding.input.text.toString().isNotEmpty()) {
            var lastNumberBefore = ""
            if (
              cursorPosition > 0 &&
                binding.input.text.toString().substring(0, cursorPosition).last() in
                  "0123456789\\$decimalSeparatorSymbol"
            ) {
              lastNumberBefore =
                NumberFormatter.extractNumbers(
            ...
            if (
              decimalSeparatorSymbol in lastNumberBefore ||
  ...
  private fun roundResult(result: BigDecimal): BigDecimal {
    ...
    if (
      MyPreferences(this).numberIntoScientificNotation &&
        (newResult >= BigDecimal(9999) || newResult <= BigDecimal(0.1))
    ) {
  ...
  private fun enableOrDisableScientistMode() {
    if (binding.scientistModeRow2.visibility != View.VISIBLE) {
      ...
      binding.scientistModeSwitchButton?.setImageResource(
  ...
  private fun updateResultDisplay() {
    lifecycleScope.launch(Dispatchers.Default) {
      ...
      if (calculation != "") {
        ...
        calculationResult =
          Calculator(MyPreferences(this@MainActivity).numberPrecision!!.toInt())
            .evaluate(calculationTmp, isDegreeModeActivated)

        // If result is a number and it is finite
        if (
          !(division_by_0 || domain_error || syntax_error || is_infinity || require_real_number)
        ) {
          ...
          if (
            !MyPreferences(this@MainActivity).numberIntoScientificNotation ||
            ...
            if (calculation != formattedResult) {
              ...
              if (isStillTheSameCalculation_autoSaveCalculationWithoutEqualOption) {
                ...
                val previousHistoryElement =
                  MyPreferences(this@MainActivity).getHistoryElementById(lastHistoryElementId)
                if (previousHistoryElement != null) {
                  ...
                  MyPreferences(this@MainActivity)
                    .updateHistoryElementById(
                      this@MainActivity,
                ...
                withContext(Dispatchers.Main) {
                  ...
                  while (
                    historySize != -1 &&
                      historyAdapter.itemCount >= historySize &&
          ...
          withContext(Dispatchers.Main) {
            if (is_infinity && !division_by_0 && !domain_error && !require_real_number) {
              if (calculationResult < BigDecimal.ZERO)
                binding.resultDisplay.text = "-" + getString(R.string.infinity)
  ...
  private fun addSymbol(view: View, currentSymbol: String) {
    ...
    if (textLength > 0) {
      ...
      val nextChar =
        if (textLength - cursorPosition > 0) binding.input.text[cursorPosition].toString()
        else "0" // use "0" as default like it's not a symbol
      val previousChar =
        if (cursorPosition > 0) binding.input.text[cursorPosition - 1].toString() else "0"

      if (
        currentSymbol != previousChar // Ignore multiple presses of the same button
        &&
          currentSymbol != nextChar &&
          previousChar != "√" // No symbol can be added on an empty square root
          &&
          previousChar !=
            decimalSeparatorSymbol // Ensure that the previous character is not a comma
          &&
        ...
        // If next character is a symbol, replace it
          ...
          nextChar.matches("[+\\-÷×^%!]".toRegex()) && currentSymbol != "%"
      ...
      if (currentSymbol == "-") updateDisplay(view, currentSymbol) else keyVibration(view)
    }
  }
    ...
    lifecycleScope.launch(Dispatchers.Default) {
      ...
      if (calculation != "") {
        ...
        if (
          ...
          withContext(Dispatchers.Main) { binding.input.isCursorVisible = false }
          ...
          withContext(Dispatchers.Main) {
            if (syntax_error) {
              ...
              binding.resultDisplay.text = formattedResult
              isEqualLastAction =
                true // Do not clear the calculation (if you click into a number) if there is an
              // error
            }
          }
        }
    ...
    if (cursorPosition != 0 && textLength != 0) {
      ...
      for (function in functionsList) {
        ...
        if (leftPart.endsWith(function)) {
          newValue =
            binding.input.text.subSequence(0, cursorPosition - function.length).toString() +
      ...
      binding.input.setText(newValueFormatted)
      binding.input.setSelection(
        (cursorPosition - 1 + cursorOffset - functionLength).takeIf { it > 0 } ?: 0
      )
    }
  }
  ...
  override fun onResume() {
    ...
    val historySize = MyPreferences(this@MainActivity).historySize!!.toInt()
    while (
      historySize != -1 && historyAdapter.itemCount >= historySize && historyAdapter.itemCount > 0
    ) {
      historyAdapter.removeFirstHistoryElement()
    }
  ...
  open fun afterInputChanged(str: String) {}
}
